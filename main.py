...
from model import LeNet_300_100, train
...
x_train, y_train, x_test, y_test=mnist_dataset(num_classes) # load preprocessed MNIST dataset

model = LeNet_300_100(input_size, num_classes) # build model
opt = keras.optimizers.Adam(learning_rate=lr)
model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy']) 
model_path="lenet300-100.h5"               #path to save original model
...
for i in range(1,100):
  ...
  pretrained_model.save("pruned_lenet_300-100.h5") #save pruned model
  pruned_loss,pruned_accuracy = pretrained_model.evaluate(x_test, y_test, verbose=0) # test pruned model
  if pruned_accuracy >=original_accuracy:   # iterative retraining exit condition
  ...
  retrain_model,history=train(pruned_model,x_train, y_train,batch_size,10,retrain_model_path) #retrain model with 10 epochs to learn important weights those are removed  during pruning
